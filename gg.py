        ...
        if action == 'reinstall':
            ...
            try:
                ...
                if check_proc.returncode != 0:
                    ...
                    await execute_lxc(f"lxc exec {container_name} -- sudo apt-get install -y software-properties-common")
                    await execute_lxc(f"lxc exec {container_name} -- sudo add-apt-repository universe -y")
                    await execute_lxc(f"lxc exec {container_name} -- sudo apt-get update -y")
                    await execute_lxc(f"lxc exec {container_name} -- sudo apt-get install -y tmate")
